...
if (Test-Path $rhythmSnapshotSummaryScriptPath) {
    ...
    $rhythmSnapshotToolHooks = @(
        ...
        'MaximumCalmPressure',
        'MaximumCalmReadabilityPressure',
        'OutputJsonPath',
        'schemaVersion',
        'ConvertTo-Json',
        'exitCode',
        ...
        'OverallEvidenceStatus',
        'thresholds',
        'ReadabilityPressure_NearStageFinal',
        'CalmEvidenceStatus',
        'BuildTemptationFlags',
        'ReleaseReliefFlags',
        'SpikeFairnessFlags',
...
if (Test-Path $rhythmWriteNextActionCmdPath) {
    $rhythmWriteNextActionCmdText = Get-Content $rhythmWriteNextActionCmdPath -Raw
    $writeNextActionHooks = @(
        'Write-RhythmSnapshotSummary.cmd',
        'Get-RhythmNextAction.ps1',
        'rhythm_snapshot_summary_last.json',
        'rhythm_next_action_last.json',
        '-OutputJsonPath'
    )
    foreach ($hook in $writeNextActionHooks) {
        if (-not $rhythmWriteNextActionCmdText.Contains($hook)) {
            $rhythmSnapshotToolMissing.Add("Write-RhythmNextAction.cmd:$hook")
        }
    }
} else {
...
if (Test-Path $rhythmNextActionTestScriptPath) {
    $rhythmNextActionTestText = Get-Content $rhythmNextActionTestScriptPath -Raw
    $nextActionTestHooks = @(
        'NO_EVIDENCE',
        'PARTIAL_EVIDENCE',
        'NEEDS_TUNING',
        ...
        'MISSING_RHYTHM_PHASE_SNAPSHOTS'
    )
    foreach ($hook in $nextActionTestHooks) {
        if (-not $rhythmNextActionTestText.Contains($hook)) {
            $rhythmSnapshotToolMissing.Add("Test-RhythmNextAction.ps1:$hook")
        }
    }
} else {
    $rhythmSnapshotToolMissing.Add('Test-RhythmNextAction.ps1 missing')
}

if (Test-Path $rhythmNextActionTestCmdPath) {
    $rhythmNextActionTestCmdText = Get-Content $rhythmNextActionTestCmdPath -Raw
    if (-not $rhythmNextActionTestCmdText.Contains('Test-RhythmNextAction.ps1')) {
        $rhythmSnapshotToolMissing.Add('Test-RhythmNextAction.cmd:Test-RhythmNextAction.ps1')
    }
} else {
...
if (Test-Path $threatReadabilityPath) {
    ...
    $releaseReliefHooks = @(
        ...
        'enableRhythmReleaseCameraExhale',
        'StartRhythmReleaseCameraExhale',
        'EvaluateRhythmReleaseCameraExhale01',
