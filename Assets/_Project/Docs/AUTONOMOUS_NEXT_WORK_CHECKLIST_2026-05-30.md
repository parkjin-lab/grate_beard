...
- Add small guardrails that catch missing rhythm phases, missing stinger assignments, spawn safety regressions, invalid state transitions, vendor import regressions, validation artifact leakage, missing planning/handoff artifacts, and validation-policy drift. `[in progress: validation, spawn safety, rhythm state, vendor ignore/tracking, log artifact, stinger slot, planning/handoff artifact, and low-touch validation policy preflight checks added]`
