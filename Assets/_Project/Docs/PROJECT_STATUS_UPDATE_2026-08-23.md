...
- Play Mode에서 좌우 이동 플립, 끊김, Spike→Release 숨이 트인다 한 방만 확인
- 리듬 페이즈 숫자, 압박, 보상값은 이번에도 손대지 않았다

## 다음 에이전트
1. Unity 콘솔에서 컴파일만 확인한다.
2. Play Mode에서 A/D 플립, Q 탭/홀드, 연막 메아리, Spike→Release 한 방을 한 번씩만 본다. 스냅샷이 없으면 숫자를 바꾸지 않는다.

## 한 줄 판정
플레이어는 가는 쪽을 보고, 메아리를 더 넓게 들을수록 더 크게 들키며, 급습이 풀리면 숨이 트이는 것을 페이즈 미터 없이 느낄 수 있다.
