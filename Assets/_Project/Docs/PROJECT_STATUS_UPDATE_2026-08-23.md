...
- Play Mode에서 벽/빵 구분, 사망 후 바로 발밑 시야, 남은 끊김만 확인
- 리듬 페이즈 숫자, 압박, 보상값, 과충전/연막 숫자는 이번에도 손대지 않았다

## 다음 에이전트
1. Unity 콘솔에서 컴파일만 확인한다.
2. Play Mode에서 벽 vs 빵, 사망 후 발밑 시야, A/D 플립, Q 탭/홀드를 한 번씩만 본다. 스냅샷이 없으면 숫자를 바꾸지 않는다.

## 한 줄 판정
벽은 이끼 돌이고 빵가루는 빛나는 빵이며, 죽으면 지도는 다시 숨지만 발밑 시야는 바로 돌아온다.
