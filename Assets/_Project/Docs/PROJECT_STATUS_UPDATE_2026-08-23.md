...
- 2–4층 클리어 그림: `StorybookSmokeAndCache` / `StorybookHoldAndListen` / `StorybookLickedTrailCache`를 S1·마녀집과 같은 Resources/Art 경로로 묶어, 클리어 책 왼쪽에 넣는다. 문구는 그대로, 파일이 없으면 예전처럼 텍스트만.

## 아직 열린 것
- Unity 6000.5.9f1 에디터 컴파일 확인
- Play Mode에서 2–4층 클리어 책 왼쪽 그림, 5층 문턱 한 장, 사망-무책만 본다
- S2–S4 PNG 바이트가 에이전트 디스크에 도착했는지 확인 (코드 훅은 연결됨)
- 리듬 스냅샷은 여전히 `NO_EVIDENCE`. 페이즈 초/압박/과충전/연막 숫자는 손대지 말 것

## 다음 에이전트
1. 세 PNG를 `Assets/_Project/Art/`와 `Resources/Story/`에 넣고 커밋한다.
2. Play Mode에서 2/3/4층 클리어 책이 매칭 그림을 보여 주는지 본다. 숫자를 바꾸지 않는다.

## 한 줄 판정
2–4층 클리어 책도 왼쪽 그림을 쓰도록 연결했다(에셋 파일만 남음).
