...
- Play Mode에서 3층 저장 후 이어하기, 새 시작 프롤로그, 엔딩 뒤 이어하기 숨김만 본다
- 리듬 스냅샷은 여전히 `NO_EVIDENCE`. 페이즈 초/압박/과충전/연막 숫자는 손대지 말 것

## 다음 에이전트
1. Unity 콘솔에서 컴파일만 확인한다.
2. Play Mode에서 3층 체크포인트 → 타이틀 `이어하기` → 3층 직행, `시작`은 프롤로그→1층인지 본다. 숫자를 바꾸지 않는다.

## 한 줄 판정
3층에서 나가도 이어하기로 바로 돌아오고, 새로 시작하면 책이 다시 열린다.
