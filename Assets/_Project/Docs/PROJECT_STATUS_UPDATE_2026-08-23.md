...
- Play Mode에서 타이틀이 책 오른장처럼 보이는지, 책 페이드/종이 소리, 스킵, 그림 없는 2–4층 페이지만 본다
- 리듬 스냅샷은 여전히 `NO_EVIDENCE`. 페이즈 초/압박/과충전/연막 숫자는 손대지 말 것

## 다음 에이전트
1. Unity 콘솔에서 컴파일만 확인한다.
2. Play Mode에서 타이틀 `시작`/`이어하기`와 책 `ShowPage`에서 짧은 종이 넘김이 들리는지 본다. 숫자를 바꾸지 않는다.

## 한 줄 판정
책을 열거나 타이틀을 고르면 짧은 종이 넘김이 들린다.
