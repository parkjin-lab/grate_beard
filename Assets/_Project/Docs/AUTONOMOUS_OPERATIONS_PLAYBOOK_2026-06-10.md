...
4. Treat `FAIL` as blocking. Treat stale log `WARN` as a refresh signal unless claiming release readiness.
5. Commit only explicit files. Do not use `git add .`.
6. Push successful commits to `origin/main`.
7. End with the current evidence, remaining risk, and the next 1-3 autonomous actions.
