...
- `Tools\Summarize-RhythmSnapshots.cmd` now summarizes rhythm snapshot phase counts, Build temptation status, Spike fairness status, and Release relief channel status for low-touch validation.
...
1. Use `Tools\Summarize-RhythmSnapshots.cmd` after rhythm snapshots exist to check whether Build has a temptation choice, Spike evidence is fair, and the first Release tuning pass keeps at least two non-text relief channels active through early/mid Release.
